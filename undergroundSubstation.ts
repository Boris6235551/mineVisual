...
export class UndergroundSubstation extends BaseMineDraw {
    constructor(p0: Point, length: number) {
        super(p0, length);
        this.name = 'UndergroundSubstation';
        // общая шина ячеек
        let i: number;
        length = length * 0.647;
...
export class UndergroundSubstationCell extends BaseMineDraw {
    constructor(p0: Point, length: number, n: number) {
        super(p0, length);
        this.name = 'UndergroundSubstationCell';
        // ячейки
